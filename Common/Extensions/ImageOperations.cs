...
namespace Common.Extensions
{
    public class ImageOperations
    {
        ...
        /// <returns></returns>
        private static string ToSeo(string incomingText)
        {
            var extension = Path.GetExtension(incomingText);
            string fileName = Path.GetFileNameWithoutExtension(incomingText);

            fileName = StringExtensions.ToSeoFilename(incomingText);
        ...
        public static string DownLoadFile(IHostingEnvironment environment, string sku, string localDirectory, string downloadUrl, string desiredName)
        {
            try
            {
                ...
                string imagePath = $"{localDirectory}{sku}/{ToSeo(desiredName)}";
        ...
        public static string SaveImage(IHostingEnvironment environment, IFormFile file,
            string sku, string localDirectory,
            int desiredWidth, int desiredHeight,
            string desiredName = "", string previousPath = "")
        {
            using (var image = Image.FromStream(file.OpenReadStream()))
            {
                if (!ValidateDimensions(image, desiredWidth - 1, desiredHeight - 1))
                    return "Not valid dimensions.";

                try
                {
                    var resizedImage = Resize(image, desiredWidth, desiredHeight, RotateFlipType.RotateNoneFlipNone);
        ...
        public static bool UploadImageToCdn(string ftpUrl, string login, string password, string sku, string fileName, string localFilePath)
        {
            ...
            if (OpenDirectoryIfNotExistsInCdn(ftpUrl, login, password, sku))
            {
                using (var client = new WebClient())
                {
                    ...
                    client.UploadFile($"ftp://{ftpUrl}/{sku}/{ToSeo(fileName)}", WebRequestMethods.Ftp.UploadFile, localFilePath);
                }
                return true;
            }
            return false;
